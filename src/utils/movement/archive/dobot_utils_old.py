...
import sys
here = os.getcwd()
there_eis = here.split('src')[0] + 'src\\characterization\\conductivity\\eis'
there_lsv = here.split('src')[0] + 'src\\characterization\\dQdV\\lsv'
sys.path.append(there_eis)
sys.path.append(there_lsv)
...
class Dobot(object):
    ...
    def setWaypoints(self):
        """
        Collection of accessible points that the arm can move through to reach destination.
        """
        return
    ...
    def transform_vector_in(self, coord, offset=False):
        translate = self.translate_vector if offset else np.zeros(3)
        return tuple( np.matmul(self.orientate_matrix, (np.array(coord)-translate)/self.scale) )

    def transform_vector_out(self, coord, offset=False):
        translate = self.translate_vector if offset else np.zeros(3)
        return tuple( self.scale * np.matmul(np.linalg.inv(self.orientate_matrix), np.array(coord)) + translate )

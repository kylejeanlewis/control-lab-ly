...
class RepRap(SerialDevice):
    ...
    def getSettings(self) -> dict[str, int|float|str]:
        ...
        settings['limit_x'] = float(settings.get('M208',{}).get('X',0).split(':')[1]) if float(settings.get('M208',{}).get('X',0).split(':')[0]) == 0 else float(settings.get('M208',{}).get('X',0).split(':')[0])*-1
        settings['limit_y'] = float(settings.get('M208',{}).get('Y',0).split(':')[1]) if float(settings.get('M208',{}).get('Y',0).split(':')[0]) == 0 else float(settings.get('M208',{}).get('Y',0).split(':')[0])*-1
        settings['limit_z'] = float(settings.get('M208',{}).get('Z',0).split(':')[1]) if float(settings.get('M208',{}).get('Z',0).split(':')[0]) == 0 else float(settings.get('M208',{}).get('Z',0).split(':')[0])*-1
        return settings
    ...
    def getStatus(self) -> tuple[str, np.ndarray[float], np.ndarray[float]]:  # TODO: Implement status check
        ...
        """
        self.clearDeviceBuffer()
        responses = self.query('M114 R', multi_out=True)
        self.clearDeviceBuffer()
